...
const OTHER_CURRENCIES = [
  ...
  'GBP','HKD','HUF','IDR','ILS','INR','MXN','MYR','NOK','NZD',
  'PEN','PHP','PLN','SAR','SEK','SGD','THB','TRY','VND','ZAR',
];

function showOtherCurrencyDropdown(query) {
  const dropdown = document.getElementById('currency-other-dropdown');
  const q = query.trim().toUpperCase();
  if (!q) { dropdown.classList.add('hidden'); return; }
  const matches = OTHER_CURRENCIES.filter(c => c.startsWith(q));
  if (!matches.length) { dropdown.classList.add('hidden'); return; }
  dropdown.replaceChildren(...matches.map(c => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.dataset.code = c;
    btn.className = 'w-full text-left px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition';
    btn.textContent = c;
    return btn;
  }));
...
async function applyOtherCurrency(code) {
  const input    = document.getElementById('input-currency-other');
  const dropdown = document.getElementById('currency-other-dropdown');
  input.value = code;
...
window.onCurrencyChange = async function() {
  const sel       = document.getElementById('input-currency').value;
  const otherWrap = document.getElementById('currency-other-wrap');
...
function renderMembers() {
  ...
  visible.forEach(member => {
    ...
    nameBtn.className = `px-3 py-1 text-sm font-medium ${isMe ? 'text-white' : 'text-gray-600'}`;
    ...
    if (!hasMemberExpenses(member.id) && !group.locked) {
      ...
      del.className = `px-2 py-1 text-sm border-l transition ${isMe ? 'border-blue-500 text-blue-200 hover:text-white' : 'border-gray-200 text-gray-400 hover:text-red-400'}`;
      del.textContent = '×';
  ...
  myExpenses.forEach(e => {
    ...
    row.innerHTML = `
      <div class="min-w-0 flex-1">
        ...
        <div class="text-sm text-gray-500">${esc(e.date)} · ${isPayer ? '<span class="text-emerald-600">我墊付</span>' : esc(payer?.name ?? '?') + ' 墊付'}</div>
      </div>
      <div class="text-right flex-shrink-0 ml-3">
        <div class="text-sm font-semibold text-gray-800">$${fmt(Math.round(share))}</div>
        ${hasFx ? `<div class="text-sm text-blue-400">${esc(e.currency)} ${fmt(shareOrig)}</div>` : ''}
      </div>
    `;
...
// 其他幣別 combobox 事件
const otherInput    = document.getElementById('input-currency-other');
const otherDropdown = document.getElementById('currency-other-dropdown');

otherInput.addEventListener('input', e => {
  showOtherCurrencyDropdown(e.target.value);
});

otherDropdown.addEventListener('click', e => {
  const btn = e.target.closest('[data-code]');
  if (!btn) return;
  applyOtherCurrency(btn.dataset.code);
});

otherInput.addEventListener('blur', () => {
  setTimeout(() => otherDropdown.classList.add('hidden'), 150);
});

let _otherCurrencyTimer = null;
otherInput.addEventListener('change', () => {
  const val = otherInput.value.trim().toUpperCase();
...
function updateCustomHint() {
  ...
  if (diff === 0) {
    ...
    hint.className = 'text-sm text-right mt-1 text-emerald-600';
  } else {
    hint.textContent = diff > 0 ? `還差 $${diff}` : `超出 $${Math.abs(diff)}`;
    hint.className = `text-sm text-right mt-1 ${diff > 0 ? 'text-orange-500' : 'text-red-500'}`;
  }
}
...
function renderCustomInputs(prevAmounts = {}) {
  ...
  checked.forEach(cb => {
    ...
    input.className = 'custom-amt flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-base focus:outline-none focus:border-blue-400 transition';
    input.placeholder = '0';
...
function loadExpenseToForm(expense) {
  ...
  if (predefined.includes(cur)) {
    ...
    sel.value = 'OTHER';
    document.getElementById('input-currency-other').value = cur;
    document.getElementById('currency-other-wrap').classList.remove('hidden');
  }
  if (cur !== 'TWD') {
...
function renderExpenseCards(highlightId = null) {
  ...
  if (!group.expenses.length) {
    container.innerHTML = `<div class="text-center py-14">
      ...
      <p class="text-gray-500 text-sm mt-1">點擊下方按鈕新增</p>
    </div>`;
    return;
  }
    ...
    .forEach(expense => {
      ...
      card.innerHTML = `
        ...
        <div class="flex-1 min-w-0">
          ...
          ${expense.currency && expense.currency !== 'TWD' ? `<p class="text-sm text-blue-500 mt-0.5">${esc(expense.currency)} ${fmt(expense.amount)}（匯率 ${Number(expense.exchange_rate).toFixed(4)}）</p>` : ''}
...
function renderSettlementHistory() {
  ...
  [...group.settlements].reverse().forEach(s => {
    ...
    div.innerHTML = `<p class="text-sm font-semibold text-gray-500">${dt}</p><p class="text-sm text-gray-500">${summary}</p>`;
    container.appendChild(div);
  });
}
